...
def run(rank, n_gpus, hps):
    ...
    train_dataset = WeTTSDataset(hps.data.train_datalist, hps.data.phn2id,
                                 hps.data.spk2id, hps.data.sampling_rate,
                                 hps.data.filter_length, hps.data.hop_length,
                                 hps.data.win_length)
    buckets = list(range(10, 10000, 100))
    ...
    if rank == 0:
        ...
        eval_dataset = WeTTSDataset(hps.data.val_datalist, hps.data.phn2id,
                                    hps.data.spk2id, hps.data.sampling_rate,
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler,
    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths,
        ...
        (z, z_p, m_p, logs_p, m_q, logs_q) = net_g(x, x_lengths, spec, spec_lengths, sid=speakers)
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                #audio_dict = {"train/predicted_audio": y_hat[0, :, :y_hat_lengths[0]]}
                image_dict = {
                    "train_slice/mel_org":
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        ...
        image_dict.update({
            "eval/gt_mel":
            utils.plot_spectrogram_to_numpy(
                mel[0, :, :spec_lengths[0]].cpu().numpy())
        })
        audio_dict.update({"eval/gt_audio": y[0, :, :y_lengths[0]]})
