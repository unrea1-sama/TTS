...
class TextMelLoader(torch.utils.data.Dataset):
    ...
    def get_text(self, text):
        text_norm = [self.tokens[x] + 1 for x in text]
        if self.add_blank:
            text_norm = commons.intersperse(
                text_norm, len(self.tokens)
...
class TextLoader(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        return self.get_text(self.audiopaths_and_text[index][2])
...
class TextCollate:
    ...
    def __call__(self, batch):
        """Collate's training batch from normalized text
        PARAMS
        ------
